...
-- Xenon Bar Interface Management
local XenonBarUI = Instance.new("ScreenGui")

XenonBarUI.Name = getgenv().UI
if gethui():FindFirstChild(getgenv().UI) then
    gethui():FindFirstChild(getgenv().UI).Parent = nil
end

XenonBarUI.Parent = gethui()
...
-- XenonBarLibrary 
local XenonBarLibrary = {}

-- Create Window Function
function XenonBarLibrary:CreateWindow(WindowProperties)
...
function Miscellaneous:CreateSlider(SliderProperties)
    ...
    -- Function to handle Input Began for both Mouse and Touch
    local function InputBegan(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            local MousePosition = UserInputService:GetMouseLocation()
            local MousePositionX = MousePosition.X
            local MousePositionY = MousePosition.Y - 36
            if MousePositionX >= SliderInteract.AbsolutePosition.X and MousePositionX <= SliderInteract.AbsolutePosition.X + SliderInteract.AbsoluteSize.X and
                MousePositionY >= SliderInteract.AbsolutePosition.Y and MousePositionY <= SliderInteract.AbsolutePosition.Y + SliderInteract.AbsoluteSize.Y then
    ...
    -- Function to handle Input Ended for both Mouse and Touch
    ...
    RunService.RenderStepped:Connect(function()
        if Slider_Dragging then
            local MousePosition = UserInputService:GetMouseLocation()
            local MousePositionX = MousePosition.X
            local Value = math.clamp((MousePositionX - SliderInteract.AbsolutePosition.X) / SliderInteract.AbsoluteSize.X, 0, 1)
            local RoundedValue = math.round(Value * (Slider_Max - Slider_Min) / Slider_Increment) * Slider_Increment + Slider_Min
        ...
        function Settings:CreateSlider(SliderProperties)
            ...
            UserInputService.InputBegan:Connect(function(Input, GameProcessed)
                ...
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    local MousePositionX = InputPosition.X
                    local MousePositionY = InputPosition.Y - 36
